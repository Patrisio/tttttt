...
export class Table {
  ...
  constructor(readOnly) {
   ...
   /**
   * Generates a menu button to remove rows and columns.
   * @return {HTMLElement}
   */
  _generateCrossButton(indexRow) {
    const button = create('td', [CSS.crossButton], {'data-index-row': indexRow});
  ...
  addRow(index = -1) {
    ...
    console.log(row, 'uuuuuuuuuuuuuuuuu');

    this._fillRow(row, this._numberOfRows - 1);
   ...
   *
   * @param {number} index - number in the array of columns, where new column to remove,-1 if remove at the end
   * @returns {HTMLElement} row
   */
  removeRow(index = -1) {
    this._numberOfRows--;
  ...
  _fillRow(row, rowIdx) {
    for (let i = 0; i <= this._numberOfColumns; i++) {
      ...
      if (i === this._numberOfColumns) {
        const deleteRowButton = this._generateCrossButton(rowIdx);
        cell.appendChild(deleteRowButton);
      }
    }
  }
